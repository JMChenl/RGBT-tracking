...
class Augmentation:
    ...
    def _crop_roi(self, image_rgb, image_t, bbox, out_sz, padding=(0, 0, 0)):
        ...
        crop_rgb = cv2.warpAffine(image_rgb, mapping, (out_sz, out_sz),
                              borderMode=cv2.BORDER_CONSTANT,
                              borderValue=padding)
        crop_t = cv2.warpAffine(image_t, mapping, (out_sz, out_sz),
                              borderMode=cv2.BORDER_CONSTANT,
                              borderValue=padding)
        return crop_rgb, crop_t
    ...
    def _shift_scale_aug(self, image_rgb, image_t,bbox, crop_bbox, size):
        im_h, im_w = image_t.shape[:2]
        ...
        image_rgb, image_t = self._crop_roi(image_rgb, image_t,crop_bbox, size)
        return image_rgb, image_t, bbox
    ...
    def __call__(self, image_rgb, image_t, bbox, size, gray=False):
        shape = image_t.shape
        crop_bbox = center2corner(Center(shape[0]//2, shape[1]//2,
                                         size-1, size-1))
        # gray augmentation
        if gray: # 0
            image = self._gray_aug(image_rgb)
            image = self._gray_aug(image_t)

        # shift scale augmentation
        image_rgb, image_t, bbox = self._shift_scale_aug(image_rgb, image_t, bbox, crop_bbox, size)
        # cv2.imwrite('img_rgb.png',image_rgb)
        # cv2.imwrite('img_t.png',image_t)
        # color augmentation
        if self.color > np.random.random():
            image_rgb = self._color_aug(image_rgb)
            image_t = self._color_aug(image_t)

        # blur augmentation
        if self.blur > np.random.random():
            image_rgb = self._blur_aug(image_rgb)
            image_t = self._blur_aug(image_t)

        # flip augmentation
        if self.flip and self.flip > np.random.random(): # 0
