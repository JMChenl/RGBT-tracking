...
class RGBT234Video(Video):
    ...
    def __init__(self, name, root, video_dir, init_rect_v, img_names_v, img_names_i,
        ...
        super(RGBT234Video, self).__init__(name, root, video_dir,
...
class RGBT234Dataset(Dataset):
    ...
    def __init__(self, name, dataset_root, load_img=False):
        super(RGBT234Dataset, self).__init__(name, dataset_root)
        with open(os.path.join(dataset_root, 'RGBT234_visible.json'), 'r') as f:
            meta_data_v = json.load(f)

        with open(os.path.join(dataset_root, 'RGBT234_infrared.json'), 'r') as f:
            meta_data_i = json.load(f)

        with open(os.path.join(dataset_root, 'RGBT234_init.json'), 'r') as f:
        ...
        for video in pbar:
            ...
            self.videos[video] = RGBT234Video(video,
                                          dataset_root,
